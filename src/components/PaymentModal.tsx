...
export function PaymentModal({ isOpen, onClose, worker }: PaymentModalProps) {
  ...
  if (!isOpen) return null;
   ...
   const handlePaymentComplete = () => {
    setStep('rating'); // Cambiar a paso de calificación en lugar de mostrar modal temporal
  };
   ...
   const handleRatingSubmit = (rating: number, comment: string) => {
    ...
    setTimeout(() => {
      setShowThanks(false);
      onClose();
      // Aquí podrías agregar lógica para guardar la calificación
    }, 2000);
  };
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
        <div className="bg-white rounded-lg w-full max-w-md mx-4">
          <RatingModal
            isOpen={true} 
            onClose={() => {}} // Deshabilitar cierre directo
